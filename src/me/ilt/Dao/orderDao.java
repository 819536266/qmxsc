...
public class orderDao {
	...
	public static JSONArray selAll(int p,int pageSize){
		String sql = "select  t_order.*,t_user.userName,t_user.trueName,t_address.province,t_address.city,t_address.area,t_address.address,t_address.phone,t_address.username from t_order,t_user,t_address where t_order.userId=t_user.id and t_address.id=t_order.addressId  order by time desc limit "+(p-1)*pageSize+","+pageSize;;
		return sel(sql);
	}
	/**
	 * 查询sql 返回json集合
	...
	public static int update(String orderId,String addressId,String PayType,String liuyan){
		System.out.println(addressId+"_-----------");
		int state = 0;
		if(PayType.equals("1")){
			//货到付款
			state = 6;
