...
public class bigTypeDao {
	...
	public static JSONArray selAll(int p,int pageSize){
		/*String sql = "select top "+pageSize+" * from t_bigType where id not in(select top "+(p-1)*pageSize+" id from t_bigType) limit ";*/
	...
	public static JSONArray nameSel(int p,int pageSize,String name){
		String sql = "select  * from t_bigType where userName like '%"+name+"%' limit" +(p-1)*pageSize +","+pageSize;
		return sel(sql);
	}
	/**
	 * 查询sql 返回json集合
	...
	public static int add(bigTypeBean u){
		String sql = "insert into t_bigType values(null,?,?,?)";
		Connection con = Conn.getCon();
