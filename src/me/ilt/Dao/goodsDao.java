...
public class goodsDao {
	...
	public static List salesTop(){
		String sql = "select  id,name,price,proPic from t_goods  order by sales desc limit 5";
	...
	public static PageBean sidPageSel(String sid,int p,String order){
		...
		if(order.equals("1")){
			sql2 = "select  id,name,price,proPic "
				...
				+" order by views desc limit "+(pb.getP()-1)*pb.getPagesize()+","+pb.getPagesize();
		}else if(order.equals("2")){
			sql2 = "select  id,name,price,proPic "
					+"from t_goods "
					+"where smallTypeId ="+sid
					+" order by id desc limit "+(pb.getP()-1)*pb.getPagesize()+","+pb.getPagesize();
		}else if(order.equals("3")){
			sql2 = "select  id,name,price,proPic "
					+"from t_goods "
					+"where smallTypeId ="+sid
					+" order by price desc limit "+(pb.getP()-1)*pb.getPagesize()+","+pb.getPagesize();
		}else if(order.equals("4")){
			sql2 = "select  id,name,price,proPic "
					+"from t_goods "
					+"where smallTypeId ="+sid
					+" order by sales desc limit "+(pb.getP()-1)*pb.getPagesize()+","+pb.getPagesize();
		}
...
public static goodsBean gwcGoodsIdSel(int id){
	...
	public static List bigTypeIdSel(int bigTypeId){
		String sql = "select  id,name,price,proPic from t_goods where bigTypeId="+bigTypeId+"  order by id desc limit "+10;
		Connection con = Conn.getCon();
		ResultSet rs = null;
		List<goodsBean> list = new ArrayList<goodsBean>();
		try {
	...
	public static JSONArray nameSel(int p,int pageSize,String name){
		String sql = "select  t_goods.*,t_smallType.name smallTypeName,t_bigType.name bigTypeName "
	...
	public static JSONArray selAll(int p,int pageSize){
		String sql = "select  t_goods.*,t_smallType.name smallTypeName,t_bigType.name bigTypeName from t_goods,t_smallType,t_bigType "
