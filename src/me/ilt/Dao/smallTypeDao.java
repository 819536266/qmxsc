...
import me.ilt.Bean.smallTypeBeanTo;
...
public class smallTypeDao {
	...
	public static JSONArray selAll(int p,int pageSize){
		String sql = "select  t_smallType.*,t_bigType.name as bigTypeName from t_smallType,t_bigType "
			+"where t_smallType.bigTypeId=t_bigType.id limit "+(p-1)*pageSize+","+pageSize;
		return sel(sql);
	}
	/**
	 * 根据昵称查询
	...
	public static JSONArray sel(String sql){
		...
		List<smallTypeBeanTo> list=new ArrayList<smallTypeBeanTo>();
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			...
			while(rs.next()) {
				...
				smallTypeBeanTo smallTypeBeanTo = new smallTypeBeanTo(id, name, bigTypeId, remarks, bigTypeName);
				list.add(smallTypeBeanTo);
			}
	...
	public static int add(smallTypeBean u){
		String sql = "insert into t_smallType values(null,?,?,?)";
		Connection con = Conn.getCon();
